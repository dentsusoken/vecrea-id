/**
 * Zod schema for Cognito user-export CSV rows.
 *
 * Duplicated from `cognito-user-import-functions/src/schemas/cognitoImportDataSchema.ts`
 * so that `user-management-apis` stays self-contained (no cross-package dependency).
 * Keep both copies in sync when columns change.
 */
...
/** Cognito CSV import standard columns (no custom attributes). Matches one PapaParse `header: true` row. */
