  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="flex items-center gap-2 mb-6">
        ...
        <span className="text-sm text-gray-900">CSVインポート</span>
      </div>

      <h1 className="text-xl font-semibold text-gray-900 mb-2">CSVインポート</h1>
      <p className="text-sm text-gray-500 mb-6">
        Cognito形式のCSVファイルをアップロードして、ユーザーを一括インポートします。
      </p>

      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-4">
        <h2 className="text-sm font-medium text-gray-700 mb-2">CSVフォーマット</h2>
        <p className="text-xs text-gray-500 mb-2">
          必須カラム: <code className="bg-gray-100 px-1 rounded">cognito:username</code>
        </p>
        <p className="text-xs text-gray-500">
          使用可能なカラム: cognito:username, email, phone_number, given_name, family_name, password_hash 等
        </p>
...
user001,user001@example.com,太郎,田中
user002,user002@example.com,花子,鈴木`}
        </pre>
      </div>
      ...
      <form onSubmit={handleImport} className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        >
          ...
          {file ? (
            ...
            <div>
              <p className="text-sm text-gray-500">クリックしてCSVファイルを選択</p>
        ...
        {result && (
          <div className={`p-4 rounded-lg border ${
            ...
            <p className="text-sm font-medium text-gray-900 mb-2">インポート結果</p>
            <div className="text-sm space-y-1">
              <p>合計: {result.totalRows} 件</p>
              <p className="text-green-700">成功: {result.successCount} 件</p>
              {result.failureCount > 0 && (
                <p className="text-red-700">失敗: {result.failureCount} 件</p>
              )}
            </div>
