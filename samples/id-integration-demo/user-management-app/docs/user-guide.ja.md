...
| フィールド | 説明 |
|---|---|
| Temporary password | 初回サインイン時に変更を求める仮パスワード（8 文字以上）。設定するとステータスが `FORCE_CHANGE_PASSWORD` になります。 |
...
3. 確認ダイアログ（Step 2）が表示されます。テキストフィールドに削除対象の **Username を正確に入力** します。
4. **Delete permanently** ボタンが有効になったらクリックします。
5. 削除が完了すると「User deleted」画面が表示されます。**Back to user list** をクリックして一覧画面に戻ります。
...
1. **CSV file** フィールドで `.csv` ファイルを選択します。
...
CSV アップロード直後は、そのアップロードのバッチ ID でフィルタリングされた状態で表示されます。**All rows** リンクをクリックするとフィルターを外してすべての行を表示できます。

### 次のステップ

ステージング内容を確認したら、画面下部の **Open user list** ボタンをクリックしてユーザー一覧画面に進みます（インポートによって Cognito に作成されたユーザーを確認できます）。
