...
サイドバー下部の **ログアウト** ボタンをクリックします。ログイン画面に遷移します。
...
| 列 | 説明 |
|---|---|
| ユーザー名 | ログイン識別子（サインイン名） |
| メール | メールアドレス |
| 氏名 | 姓・名（family_name + given_name） |
| ステータス | アカウントのステータス（[詳細は §9 参照](#9-ユーザーステータス一覧)） |
| 有効 | アカウントが有効かどうか |
...
ユーザー一覧画面の **ユーザーを追加** ボタンをクリックして開きます。
...
| フィールド | 説明 |
|---|---|
| ユーザー名 | Cognito のサインイン識別子。作成後は変更できません。 |
...
| 姓 / 名 | プロフィール属性 |
| メールアドレス | メールアドレス |
| 電話番号 | E.164 形式で入力します（例: `+819012345678`）。 |
| 初期パスワード | 省略すると Cognito が自動生成します。設定するとステータスが `FORCE_CHANGE_PASSWORD` になります。 |
| 招待メールを送信しない | チェックすると Cognito からの招待メールを送信しません。 |

### 作成

**作成** ボタンをクリックします。作成が成功するとユーザー一覧画面へ遷移します。
...
以下の項目を変更して **保存** ボタンで保存できます。保存後はユーザー一覧画面へ遷移します。

| フィールド | 説明 |
|---|---|
| 姓 / 名 | プロフィール属性 |
| メールアドレス | メールアドレス |
| メール確認済みにする | チェックするとメール確認済みフラグを true にします |
| 電話番号 | 電話番号（E.164 形式） |
| アカウントを有効にする | チェックを外すとユーザーを無効化します |

---

## 5. ユーザー削除

ユーザー編集画面の **ユーザーを削除** ボタンから削除できます。
...
ユーザー一覧画面の **CSVインポート** ボタンをクリックして開きます。CSV ファイルを使って複数のユーザーをステージングテーブルに一括登録できます。
...
サイドバーの **ステージング** をクリックしてアクセスします。CSV インポート後の一時保存データを確認・削除できます。
...
| 表示 | 説明 |
|---|---|
| インポート済み | ユーザーへの移行が完了済み |
| 初回ログイン待機中 | まだ移行が完了していない状態 |
| エラー | インポート処理中にエラーが発生した |

### 一括削除

チェックボックスで行を選択し、**〇 件削除** ボタンをクリックすると選択したステージングデータを削除できます。
...
2. **全データをリセット** ボタンが有効になったらクリックします。
...
| ステータス | 表示名 | 説明 |
|---|---|---|
| `CONFIRMED` | 確認済み | メール確認済みの通常アカウント |
| `UNCONFIRMED` | 未確認 | まだ確認が完了していないアカウント |
| `FORCE_CHANGE_PASSWORD` | パスワード変更待ち | 仮パスワードが設定されており、初回サインイン時にパスワード変更が必要 |
| `RESET_REQUIRED` | リセット必要 | パスワードのリセットが必要な状態 |
